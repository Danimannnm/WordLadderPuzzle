def extract_word_from_entry(entry):
    ...
    Returns None if the word starts with a hyphen or contains a hyphen.
    """
    ...
    if lines:
        ...
        if tokens:
            word = tokens[0]
            # Filter out words that start with a hyphen or contain a hyphen
...
def main():
    ...
    output_filename = "oxford_words.txt"    # Output file with one word per line
